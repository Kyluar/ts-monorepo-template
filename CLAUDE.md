...
- Five GitHub Actions workflows in `.github/workflows/`:
  ...
  - `security.yml` — three jobs: Semgrep SAST (`semgrep-scan`, outputs SARIF uploaded to GitHub Code Scanning) + TruffleHog secret scan (`trufflehog-scan`) + Trivy SCA (`trivyFsScan` for npm deps, `trivyImageScan` for Docker images with auto-discovery of `apps/*/Dockerfile`, fails on HIGH/CRITICAL) on PRs
